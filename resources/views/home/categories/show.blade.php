...
@section('script')
    <script>
        function filter() {
            ...
            let sotrBy = $('#sort-by').val();

            if (sotrBy == "default") {
                $('#filter-sort-by').prop('disabled', true);
            } else {
                $('#filter-sort-by').val(sotrBy);
            ...
            // console.log($('#filter-variation').val(), '2');
            ...
            <div class="row flex-row-reverse text-right">
                ...
                <div class="col-lg-3 order-2 order-sm-2 order-md-1">
                    <div class="sidebar-style mr-30">
                        ...
                        <div class="sidebar-widget mt-30">
                            <h4 class="pro-sidebar-title">{{ $varition->name }} </h4>
                            <div class="sidebar-widget-list mt-20">
                                <ul>
                                    @foreach ($varition->varitionValues as $value)
                                        <li>
                                            <div class="sidebar-widget-list-left">
                                                            ...
                                                            <li>
                                                                @auth
                                                                    @if ($product->checkUserWishlist(auth()->id()))
                                                                        <a
                                                                            href="{{ route('home.whishlist.remove', ['product' => $product->id]) }}"><i
                                            ...
                                            <div class="pro-details-size w-50">
                                                ...
                                                <select class="form-control variation-select">
                                                    @foreach ($product->variations()->where('quantity', '>', 0)->get() as $variation)
                                                        <option
                                                            value="{{ json_encode($variation->only(['id', 'quantity', 'is_sale', 'price', 'sale_price'])) }}"
